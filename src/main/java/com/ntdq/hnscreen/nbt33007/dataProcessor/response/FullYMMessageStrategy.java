...
public class FullYMMessageStrategy implements ReceiveMessageStrategy {
    ...
    public void receiveMessage(Channel channel, MessageDetail messageDetail) {
        ...
        for (int i = 0; i < infoBody.length; i+=5) {
            ...
            log.info("遥脉值{}：{}",count,ycValue);
            count++;
        }

        log.info("------------Message:上送全遥脉 end-------------");
    }
}
